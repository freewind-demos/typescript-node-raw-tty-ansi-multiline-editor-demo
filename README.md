...
本 Demo 以 **Shift+Enter** 插入换行、**Enter** 提交整段：优先读 `key.shift`；若终端在 raw 下不把修饰键带上，则把 **`key.sequence` 为「ESC 开头 + CR/LF 结尾」** 的 Return 也视为 Shift+Enter（常见 CSI 修饰键序列）。
...
1. `\x1b[?25l` 隐藏光标（减轻闪烁）。
2. 若光标曾被 `placeDraftCursor` 移进草稿行，下次不能从「行内」直接 `nA`，否则会误进上面的 transcript。画完草稿后在**草稿块最下一行之下**用 `\x1b[s` 存锚点，每次重画前 `\x1b[u` 回到该点，再 `\x1b[nA` 上移 `n` 行到草稿块首行，`\x1b[0J` 清到屏末。
...
你按 **Enter** 时，本 Demo 把当前缓冲拼成一个大字符串交给命令处理或回显；按 **Shift+Enter** 时只在缓冲里插入换行，不提交。粘贴时，终端通常一次性塞入许多字符，有的还带换行；`emitKeypressEvents` 会尽量拆成事件，程序把这些字符按顺序写进缓冲即可。
